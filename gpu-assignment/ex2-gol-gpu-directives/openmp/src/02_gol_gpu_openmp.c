...
void game_of_life(struct Options *opt, int *current_grid, int *next_grid, int n, int m){
    ...
    #pragma omp target teams distribute parallel for collapse(2) private(neighbours, n_i, n_j) 
    for(int i = 0; i < n; i++){
        for(int j = 0; j < m; j++){
...
void game_of_life_stats(struct Options *opt, int step, int *current_grid){
    ...
    #pragma omp target teams distribute parallel for collapse(2) map(num_in_state) map(to:current_grid[0:n*m]) reduction(+:num_in_state)
    for(int j = 0; j < m; j++){
        for(int i = 0; i < n; i++){
...
int main(int argc, char **argv)
{
    ...
    #pragma omp target enter data map(to: grid[0:n*m]) map(alloc: updated_grid[0:n*m])
    {
		...
		// swap current and updated grid only
        	tmp = grid;
        	grid = updated_grid;
        	updated_grid = tmp;
    ...
    #pragma omp target exit data map(from: grid[0:n*m]) map(delete:updated_grid[0:n*m])
